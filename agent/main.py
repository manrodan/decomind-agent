...
root_agent = Agent(
    ...
    instruction=(
        ...
        "       floor, has_elevator, is_attic, energy_rating, exterior\n"
        "       (los que el usuario haya mencionado).\n"
        "     Cita el `combined_factor` hedónico.\n"
        "\n"
        "  TRIANGULACIÓN DE FUENTES (muy importante para la credibilidad):\n"
        "  Reporta SIEMPRE las dos fuentes oficiales en paralelo:\n"
        "    - Notariado (precio REAL de transacción): X €/m² · N ventas\n"
        "    - MITMA (valor tasado de referencia): Y €/m²\n"
        "  Y calcula la convergencia = min(X,Y)/max(X,Y) en %. Si >85%, di\n"
        "  'alta convergencia → valoración robusta'. Si <85%, explica el gap\n"
        "  (zona prime → transacción supera tasación; zona floja → al revés).\n"
        "\n"
